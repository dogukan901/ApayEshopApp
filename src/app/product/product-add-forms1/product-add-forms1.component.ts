...
export class ProductAddForms1Component implements OnInit {
  ...
  add(form: NgForm) {
    this.productService.addProduct(this.model).subscribe(data => {
      this.alertifyService.success(data.name + "successfully added as a new Product");
    });
    // console.log(form.value.name)//written this code to check if adding method works
  }
}
